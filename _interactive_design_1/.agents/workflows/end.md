...
5. **Git Protocol & Indexing**: Execute a semantic Git commit. If a remote repository is configured (`git remote -v` returns a result), run `git push` to trigger the pre-push hook for secret scanning and memory sync.
   📦 **Memory Consolidation**: If the knowledge base has grown significantly, run deduplication (INSIDE container):
   - `docker exec -i -interactive-design-1-workspace bash -c "source .venv/bin/activate && python scripts/consolidate_memory.py" || echo "⚠️ consolidation failed, continuing..."`
